...
export default function GenerateInvoicePage() {
  ...
  const [analyzedWIPData, setAnalyzedWIPData] = useState<WIPEntry[]>([]);
  ...
  const handleWIPFileAnalyzed = async (data: any) => {
    ...
    try {
      ...
      if (analyzedData.error) {
        throw new Error(analyzedData.error);
      }
      
      if (!analyzedData.data) {
        throw new Error('No data returned from analysis');
      }
      
      console.log('Analyzed WIP data:', analyzedData.data);
      
      // Convert Gemini response to WIPEntry array
      ...
      Object.entries(analyzedData.data).forEach(([clientId, clientData]: [string, any]) => {
        ...
        Object.entries(clientData.projects).forEach(([serviceType, projectData]: [string, any]) => {
          ...
          // Create a WIPEntry for each service type
          const wipEntry: WIPEntry = {
            ...
            timeInMinutes: projectData.totalHours * 60,
            partner: "Unknown", // You might want to get this from somewhere
            hourlyRate: projectData.totalAmount / projectData.totalHours,
            associatedDailyIds: [],
            subEntries: projectData.subtasks.map((task: any) => ({
              id: Date.now() + Math.random(),
              description: task.description,
              dates: [],
              timeInMinutes: task.hours * 60,
              associatedDailyIds: []
            })),
            startDate: Date.now(),
          ...
          wipEntries.push(wipEntry);
        });
      });
      
      setAnalyzedWIPData(wipEntries);
    } catch (error) {
  ...
  return (
    <div className="p-6 space-y-8 max-w-7xl mx-auto">
      ...
      <div className="bg-white dark:bg-[#1f2937] rounded-xl border border-gray-200 dark:border-[#374151] shadow-lg p-6">
        ...
        {analyzedWIPData.length > 0 && (
          <div className="mt-6 space-y-4">
            <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300">Analyzed WIP Data</h3>
            <div className="space-y-4">
              {analyzedWIPData.map((entry: WIPEntry) => 
                renderClientCard(entry.client, {
                  projects: {
                    [entry.project]: {
                      entries: [entry],
                      totalHours: entry.hours,
                      totalAmount: entry.hours * entry.hourlyRate
                    }
                  },
                  totalAmount: entry.hours * entry.hourlyRate,
                  totalHours: entry.hours
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center">
              <span className="px-2 bg-gray-50 dark:bg-[rgb(17,17,17)] text-sm text-gray-500">
                {analyzedWIPData.length > 0 ? 'or select from existing WIP' : 'or select from existing WIP'}
              </span>
            </div>
          </div>
